...
export default {
  ...
  methods: {
    ...
    async getUserList() {
      /* alert(this.queryInfo.pagesize)
       * console.log(this.queryInfo)
       */
      const { data: res } = await this.$http.post('user/show', this.queryInfo)
      if (res.status !== 200) {
    ...
    addUser() {
      this.$refs.addFormRef.validate(async valid => {
        if (!valid) return
        // 可以发起添加用户的网络请求
        const { data: res } = await this.$http.put('user/add', this.addForm)
        if (res.status !== 200) {
